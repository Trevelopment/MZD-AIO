...
function printTXTTest(device, callback) {
  const filename = copyToTemp(path.resolve(path.join(__dirname, '../assets/printer/test.txt')));
...
function printPDFTest(device, callback) {
  const filename = path.resolve(path.join(__dirname, '../assets/printer/test.pdf'));
