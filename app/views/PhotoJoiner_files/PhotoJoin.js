...
function JoinPhotos () {
  ...
  fs.writeFile(`${getBackground}`,
    ...
    (err) => {
      ...
      $('#Panel1').hide()
      outImage.src = `${getBackground}`
      $('#imgJoined').attr('src', `${getBackground}`)
    })
}
...
function CreateImagePreview (imgpath) {
  ...
  if (style == 1) { img.setAttribute('height', 100) } else if (style == 2) { img.setAttribute('width', 100) } else if (style == 3) { img.setAttribute('width', 100) }
  var thumb = document.createElement('div')
  thumb.setAttribute('class', 'thumb')
