...
then
	...
	log_message "===*** UNINSTALL DIAGMENU W. 1 SEC. PRESS AT CLOCK IN DISPLAY SETTINGS MENU ... ***==="
...
if [ -e /jci/gui/apps/diag/js/diagApp.js.org ] || [ -e /jci/gui/apps/diag/js/diagApp.js.org ]
then
	if [ "${DEL_BAKUPS}" = "1" ]
	then
...
then
	...
	log_message "=== Restored original FuelConsumptionCtrl.png from backup FuelConsumptionCtrl.png.org ==="
	if [ "${DEL_BAKUPS}" = "1" ]
	then
...
then
	log_message "====*** RREVERTING /jci/scripts/jci-fw.sh ***====="
...
then
	...
	log_message "=== /jci/gui/common/controls/NowPlaying4/js/NowPlaying4Ctrl.js Restored from backup  ==="
	if [ "${DEL_BAKUPS}" = "1" ]
	then
...
log_message "=======********  UNINSTALL VIDEOPLAYER & SPEEDOMETER, ANDROID AUTO & CASTSCREEN RECIEVER ... ********======="
...
rm -f /jci/opera/opera_dir/userjs/speedometer-startup.*
...
rm -fr /tmp/mnt/data_persist/dev/
...
log_message "===*** END UNINSTALLATION OF REMOVE SAFETY WARNING FROM REVERSE CAMERA ***==="
log_message " "
...
then
	...
	log_message "===*** RESTORE ALL INFOTAINMENT COLORS AND IMAGES (BACK TO RED) ... ***==="
