...
*/
    ...
    showHelp: function(seconds) {
        ...
        setTimeout(function(){ canvas.hidden = 'true' }, seconds * 1000);  // delay closing of window
    },
	...
	updateSpeedoGraph: function() {
		...
		var region = this.getRegion(),
			scale = this.scales[region], //  || this.scales.eu,
        ...
        if (this.speedMode=='graph') {   // draw speed graph
    		...
    		// 	ctx.setLineDash([0, 0]);   // Peter-dk: Dashed lines do work in the car
    			...
    			this.statistics.averageSpeeds.forEach(function(avg, index) {
    				...
    				var x = 260 - (index * ds),
    					y = 150 - DataTransform.scaleValue(avg, [0, scale.scaleMaxSpeed], [0, 150]);
    	...
    	//	$('<div/>').addClass('divider').appendTo(this.speedoGraph);
            ...
            var stH = 135; //Start height, y = 0

            var xOffset = 5;  // x start      
            var xScale = 12;  // distance btw. vertical bars
        ...
        ctx.strokeStyle = 'rgba(255, 255, 255, 0.75)';
        ctx.beginPath();
            ctx.moveTo(206, 36); ctx.lineTo(206, 76); ctx.lineTo(226, 56); ctx.lineTo(206, 36);
        ctx.stroke();
 ...
 */     		
    ...
    */
	...
	setSpeedPosition: function(speed) {
        ...
        // Peter-dk
        var plotMaxSpeed = this.statistics.topSpeed;
        ...
        var R = 117;  var xc = 118;  var yc = -70;
        var alfa = - (plotMaxSpeed + 60) * Math.PI / 180;
        var x2 = xc + R * Math.sin(alfa);
        var y2 = yc + R * Math.cos(alfa);
	...
	setGPStime: function(timestamp) {
        ...
        var normal = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365];
        var leap = [0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335, 366];
        var mo = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];

        var month, year;
        var days = Math.floor((timestamp - 1451602800) / 3600 / 24); // subtract epoc of 1/1-2016 and convert to days since 1/1-2016
    ...
    */
    ...
    setCurTime: function(curTime) {
        ...
        var mins = Math.round((curTime - this.startTime)/600);
