...
usbaudioApp.prototype.appInit = () => {
  ...
  this._umpButtonConfig = {};

  // UMP Default config
  const selectCallback = this._umpDefaultSelectCallback.bind(this);
  const holdStartCallback = this._umpHoldStartCallback.bind(this);
  const holdStopCallback = this._umpHoldStopCallback.bind(this);
  const slideCallback = this._umpSlideCallback.bind(this);
  // @formatter:off
...
usbaudioApp.prototype._populateFolders = function(tmplt, data, dataList, index, viewType) {
  ...
  const listConfig = this._initDataList(dataList, context, tmplt, data, currentView);
...
usbaudioApp.prototype._populateList = function(tmplt, data, context, dataList, scrollIndex) {
  ...
  const listConfig = this._initDataList(dataList, context, tmplt, data);
...
usbaudioApp.prototype._createRequest = () => {
  ...
  if (this._hasContextPayload()) {
    ...
    const mdInfoList = this._createMdInfo( mdInfo );
    const sortSettings = this._payloadTable[this._currentContextId].sort_settings;
...
usbaudioApp.prototype._countAlbums = function(artistName, action, artistId, fromVui, contextPath) {
  ...
  let mdInfo = [{value: artistName, type: this._mdFilter.USBM_MetadataType_ArtistName, item_id: artistId}];
...
usbaudioApp.prototype._countArtists = function(albumName, action, fromVui, contextPath) {
  ...
  const mdInfo = [{value: albumName, type: this._mdFilter.USBM_MetadataType_AlbumName, item_id: 0}];
    ...
    if (params.params.total_count > 1) {
      ...
      if ((this._connectedDevs.selectedDevType == 'UMASS') ||
        ...
        if (action == 'browse') {
          ...
          const mdType = [this._mdFilter.USBM_MetadataType_ObjectName];
          const mdInfo = [{value: artistName, type: this._mdFilter.USBM_MetadataType_ArtistName, item_id: 0}];
          const sortSettings = [{metadata_type: this._mdFilter.USBM_MetadataType_ObjectName, sort_order: this._soFilter.USBM_SortOrder_AlphaAscending}];
