...
NowPlaying4Ctrl.prototype._createStructure = function() {
  ...
  for (let hdSubstNum = 1; hdSubstNum <= this._constants.MAX_HD_SUBSTATION_COUNT; hdSubstNum++) {
  ...
  for (let rmStarNum = 1; rmStarNum <= this._constants.MAX_RATING_METER_STAR_COUNT; rmStarNum++) {
...
NowPlaying4Ctrl.prototype._updateHdSubstationStyles = function() {
  if (this.properties.hdConfigObj) {
    ...
    for (let i = 1; i <= this._constants.MAX_HD_SUBSTATION_COUNT; i++) {
      ...
      let hdSubst = this.properties.hdConfigObj.hdSubstList[i - 1];
...
NowPlaying4Ctrl.prototype.setNowPlayingConfig = function(config) {
  ...
  for (const i in this._styles[this.properties.ctrlStyle]) {
  ...
  if (elt) {
    if (txt) {
      if (utility.toType(txt) == 'string') {
        const spanCloseIdx = txt.search('</span>');
        if (spanCloseIdx >= 0) {
...
NowPlaying4Ctrl.prototype.setHdConfig = function(hdConfigObj) {
  ...
  if (hdConfigObj) {
    ...
    if ((this.properties.ctrlStyle === 'Style2') ||
      ...
      this.properties.hdConfigObj.hdSubstList = [];
      ...
      if (hdConfigObj.hdSubstPresetNum &&
        ...
        for (let i = 0; i < this._constants.MAX_HD_SUBSTATION_COUNT; i++) {
...
NowPlaying4Ctrl.prototype.setCtrlSubtitle = function(ctrlSubtitleObj) {
  ...
  if (this.properties.ctrlStyle === 'Style7') {
    let styleName = this._styles[this.properties.ctrlStyle]['_ctrlSubtitle'];

    if (ctrlSubtitleObj) {
      this.properties.ctrlSubtitleObj = {};
...
NowPlaying4Ctrl.prototype.setAudioTitle = function(audioTitleObj) {
  ...
  let styleName = this._styles[this.properties.ctrlStyle]['_audioTitleFrame'];

  if (audioTitleObj && !this.properties.unformattedTextObj) {
    this.properties.audioTitleObj = {};
...
NowPlaying4Ctrl.prototype.setDetailLine1 = function(detailLine1Obj) {
  ...
  let styleName = this._styles[this.properties.ctrlStyle]['_detailLine1Frame'];

  if (detailLine1Obj && !this.properties.unformattedTextObj) {
    this.properties.detailLine1Obj = {};
...
NowPlaying4Ctrl.prototype.setDetailLine2 = function(detailLine2Obj) {
  ...
  let styleName = this._styles[this.properties.ctrlStyle]['_detailLine2Frame'];

  if (detailLine2Obj && !this.properties.unformattedTextObj) {
    this.properties.detailLine2Obj = {};
...
NowPlaying4Ctrl.prototype.setDetailLine3 = function(detailLine3Obj) {
  ...
  let styleName = this._styles[this.properties.ctrlStyle]['_detailLine3Frame'];

  if (detailLine3Obj && !this.properties.unformattedTextObj) {
    this.properties.detailLine3Obj = {};
...
NowPlaying4Ctrl.prototype.setUnformattedText = function(unformattedTextObj) {
  ...
  let styleName = 'NowPlaying4CtrlUnformattedTextFrame';

  if (unformattedTextObj && this.properties.ctrlStyle === 'Style7') {
    this.properties.unformattedTextObj = {};
...
NowPlaying4Ctrl.prototype.handleControllerEvent = function(eventId) {
  ...
  // Pass-through
  if (this.umpCtrl) {
    const response = this.umpCtrl.handleControllerEvent(eventId);
    return response;
  }
};
