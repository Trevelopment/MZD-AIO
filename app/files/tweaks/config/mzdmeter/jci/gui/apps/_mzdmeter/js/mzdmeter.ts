...
const egt_temp_lowest = 5;
const egt_temp_normal_start = 50;
const egt_temp_normal_end = 57; // default = 58  *** , high-sense = 56, dangerous-zone = 65
const egt_temp_lvl1 = 65; // default = 65 , high-sense = 60, dangerous-zone = 70
const egt_temp_lvl2 = 70; // default = 75 , high-sense = 65, dangerous-zone = 75
...
let speedRestrictAlarm = false;
let showRestrict = false;

let proc_1_check = false;
let proc_2_check = false;
let proc_3_check = false;

let msgDebug = '';
// let speed120Set = null;
...
$(document).ready(function() {
  ...
  function retrieveValueAll(action) {
    /* let speedometerWsValue = new ReconnectingWebSocket('ws://127.0.0.1:9969/', null, {debug: false, timeoutInterval: 5000, reconnectInterval: 500});*/
    const speedometerWsValue = new WebSocket('ws://127.0.0.1:44944/');
  ...
  function retrievedataHS(action) {
    /* let speedometerWsHS = new ReconnectingWebSocket('ws://127.0.0.1:55544/', null, {debug: false, timeoutInterval: 5000, reconnectInterval: 500});*/
    /* let speedometerWsHS = new WebSocket('ws://127.0.0.1:55544/');*/
    const speedometerWsHS = new WebSocket('ws://127.0.0.1:44944/'); // Try to use same port to utilize the resource on backend process
  ...
  function retrievedataVihicleSpeedAndRPM(action) {
    /* let speedometerWsVih = new ReconnectingWebSocket('ws://127.0.0.1:55574/', null, {debug: false, timeoutInterval: 5000, reconnectInterval: 500});*/
    const speedometerWsVih = new WebSocket('ws://127.0.0.1:44944/');
    speedometerWsVih.onmessage = function(event) {
      if (!proc_1_check) proc_1_check = true;
      const res = event.data.split('#');

      const speed = res[0];
      const RPM = parseInt(res[1])*2;
      updateEngineRPM(RPM);
  ...
  function updateIntakeAirTemp(IntakeAirTempIn) {
    let needPos = 0;
    let IntakeAirTemp = $.trim(IntakeAirTempIn);
  ...
  function updateTripTime() {
    if (alreadyGetTimeFg) {
      ...
      const days = Math.floor(totalTripSeconds / 86400);
      const hours = Math.floor((totalTripSeconds - (days * 86400)) / 3600);
      let minutes = Math.floor((totalTripSeconds - (days * 86400) - (hours * 3600)) / 60);
  ...
  function updateIdleTime() {
    if (alreadyGetTimeFg) {
      const days = Math.floor(totalIdleSeconds / 86400);
      const hours = Math.floor((totalIdleSeconds - (days * 86400)) / 3600);
      let minutes = Math.floor((totalIdleSeconds - (days * 86400) - (hours * 3600)) / 60);
  ...
  function updateVehicleSpeed(currentSpeed) {
    const currentSpeedUse = $.trim(currentSpeed);
  ...
  function updateEngineRPM(currentRPM) {
    const currentRPMShow = -120 + parseInt((currentRPM * 0.03).toFixed(0));
  ...
  function updateRPMLimit() {
    const currentRPMLimitShow = -120 + parseInt((rpmAlarmLimitValue * 0.03).toFixed(0));
  ...
  function updateGPSAltitude(currentGPSalt) {
    const currentGPSalt = $.trim(currentGPSalt);
  ...
  function updateCompassGPS(currentGPCompass) {
    const currentGPCompass = $.trim(currentGPCompass);
    let heading = 'N';
    if ($.isNumeric(currentGPCompass) && currentGPCompass != lastGPSheadingValue) {
      ...
      let degShow = -Math.abs(currentGPCompass)+180+compassCalibrate;
      if (degShow >= 0) {
        degShow = 360 - degShow;
      } else {
  ...
  function updateGPSLatitude(currentGPSlat) {
    const currentGPSlat = $.trim(currentGPSlat);
  ...
  function updateGPSLongitude(currentGPSlon) {
    const currentGPSlon = $.trim(currentGPSlon);
