...
function StatusBarCtrl(uiaId, parentDiv, controlId, properties)
{
  ...
  this._iconDivs = {}; // (Object) Stores references to the Status Bar icon divs
  ...
  this._CPAAParameter = {
    'left': '58px',
    'width': '628px',
    'hideIcons': ['Batt', 'WifiSignal', 'Bluetooth', 'PhoneSignal'],
    '_left': '0px', // (String) backup left
    '_width': '800px', // (String) backup width
  };
  // @formatter:on
...
StatusBarCtrl.prototype._createStructure = function()
{
  ...
  const iconsR0 = ['Traffic', 'Roaming'];
  const iconsR1 = ['Message', 'WifiSignal', 'Batt', 'Bluetooth'];
  ...
  for (const row in iconsMatrix)
  {
    ...
    for (const col in iconsMatrix[row])
    {
      ...
      const name = iconsMatrix[row][col];
...
StatusBarCtrl.prototype.setupCPAAMode = function(enable)
{
  ...
  this._lastApp = framework.getCurrentApp();
};
...
StatusBarCtrl.prototype.setAppNameId = function(uiaId, stringId, subMap)
{
  ...
  const text = framework.localize.getLocStr(uiaId, stringId, subMap);
...
StatusBarCtrl.prototype.showRemoteUiButton = function(config)
{
  if (config)
  {
    ...
    if (this.homeBtn.divElt.style.display === 'none') // checks if the Home button is not shown.
    {
      ...
      const remoteUiBtnProperties = {
        'buttonBehavior': 'shortAndHold',
        'canStealFocus': true,
        'selectCallback': config.selectCallback,
        'onFocusCallback': config.onFocusCallback,
        'icon': config.iconPath,
        'appData': config.appData,
        'appName': config.appName,
        'enabledClass': remoteUIBtnClassPrefix + '_En',
        'focusedClass': remoteUIBtnClassPrefix + classType + '_focused',
