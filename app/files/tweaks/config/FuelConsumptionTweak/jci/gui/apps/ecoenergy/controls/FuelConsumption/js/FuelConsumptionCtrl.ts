...
//    log.debug("FuelConsumptionCtrl: _init() called...");
  ...
  this._CDFEGraphBarValues = [];
  this._CDFEGraphLineValues = [];
  this._CDFEDiscValues = [];
  /** **********************************Historical data displayed by CDFE graph Right***********************************/

  this._CDFEGraphBarValuesRight = [];
  this._CDFEDiscBarValuesRight = [];

  /** **********************************End of Historical data displayed by CDFE graph Right ***********************************/
  // Historical/current data displayed by CFER graph
  this._CFERGraphBarValues = [];
...
//    log.debug("FuelConsumptionCtrl: _createStructure() called...");
  ...
  for (let i = 1; i <= this._totalCDFEBarsRight; i++) {
    ...
    const CDFEBarCapRight = document.createElement('div');
  ...
  for (let j = 0; j < this._totalCFERBars; j++) {
    const curCFERBar = document.createElement('div');
  ...
  const CurrentCFERBarCap = document.createElement('div');
...
//    log.debug("FuelConsumptionCtrl: _addCDFEGraphAxisLabels() called...");
  ...
  const yUnitLabel = document.createElement('span');
  yUnitLabel.style.position = 'absolute';
  yUnitLabel.style.width = '100px';
  yUnitLabel.style.top = '90px';

  const yZeroLabel = document.createElement('span');
  ...
  const xAxisLabelMinuteText = this._translateString(this.properties.xAxisLabelMinuteId,
...
//    log.debug("FuelConsumptionCtrl: _drawCDFELineGraphInterval() called...");
  ...
  if ((typeof(startIdx) === 'number') &&
    ...
    let leftX = (this._CSSConstants['CDFEGraphBarSpacing'] +
                    (this._CSSConstants['CDFEGraphBarWidth'] / 2)) +
                    (barInterval * startIdx);

    // Center of next bar
    let rightX = leftX + barInterval;
    let leftY;
    let rightY;
...
FuelConsumptionCtrl.prototype.updateCurrentCumulativeFuelGraph = function(currentBarValue, unitRange) {
  ...
  const currentValueDiv = document.getElementById('CFERBarValueCurrent');
...
FuelConsumptionCtrl.prototype.setFuelEfficiency = function(fuelEfficiencyData) {
  ...
  this.properties.fuelEfficiencyData = {};
  ...
  if (fuelEfficiencyData &&
    ...
    const fuelEfficiencyUnitText = this._translateString(this.properties.fuelEfficiencyData.fuelEfficiencyUnit,
