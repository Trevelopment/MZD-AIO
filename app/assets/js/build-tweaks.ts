...
import {approot, builddir, colordir, getBackground, langObj, logFileName, replaceInFile, settings, varDir} from './index';

// First line of AIO loh
let AIO_LOG = `# __MZD-AIO-TI__ ${app.getVersion()}| MZD All In One Tweaks Installer\n#### AIO COMPILATION LOG - ${Date()}\r\n___\n- *START!*\n`;
let AIO_LOG_HTML = `<button class="w3-btn w3-hover-teal w3-display-bottomright w3-margin" onclick="saveAIOLogHTML()">Save Log (HTML)</button><div id="aio-comp-log" class="aio-comp-log"><h1><b>MZD-AIO-TI ${app.getVersion()}</b> | MZD All In One Tweaks Installer</h1><br><h4> AIO COMPILATION LOG - ${Date()}</h4><hr><div><ul><li><b><i>START!</i></b></li>`;
let userView = document.getElementById(`userLogView`);
let fileCount = 0; // Number of files in the Queue
let opsComplete = false; // Flag to signal writing of tweaks.sh file is completed
let filesComplete = true; // Flag to signal the files have all been copied
let errFlag = false; // Error flag
let copySwapfile = false; // Swapfile flag
let tweaks2write: string[] = []; // Queue for order consistantcy writing tweaks.sh (Always starts with 00_intro.txt)
let tmpdir = path.normalize(path.join(persistantData.get('copyFolderLocation', app.getPath('desktop')), '_copy_to_usb')); // Place to hold USB drive files before copying
let themeColor = 'Red'; // Default color
let tweaksFileName = 'tweaks'; // Filename for tweaks.sh
let userOps: any;
let swapdest = '';
let keeplog = true;
...
\**********                                     */
export const buildTweakFile = (user: any, apps: any) => {
  userOps = user;
...
function buildTweaksConfig(user: any, apps: any) {
  setTimeout(() => {
...
function addBlankAlbumArt(user: any, inStr: any) {
  const outStr = fs.createWriteStream(`${tmpdir}/config/blank-album-art-frame/jci/gui/common/images/no_artwork_icon.png`);
  outStr.on('open', () => {
...
function customTheme(color: any, user: any) {
  if (user.customTheme) {
    if (user.customTheme[0].substr(user.customTheme.length - 4) === 'jci') {
      aioLog(`Copying ${user.customTheme} Theme Folder`);
      copydir(`${user.customTheme}`, `${tmpdir}/config/color-schemes/theme/jci/`, (stat: any, filepath: any, filename: any) => {
...
function setColor(color: any, user: any) {
...
function buildTweak(user: any) {
  ...
  if (user.mainOps.includes(10) || user.mainOps.includes(110)) {
    const offBG = user.mainOps.includes(10) ? `${varDir}/OffScreenBackground.png` : path.resolve(approot, '../background-images/default/OffScreenBackground.png');
    const inStrOff = fs.createReadStream(offBG);
    inStrOff.on('error', (err: any) => {
      aioLog('ERROR: No Off-Screen Background Chosen... ', err);
    });
    inStrOff.on('open', () => {
      addTweak(`00_offbackground-${user.mainOps.includes(10) ? 'i' : 'u'}.txt`);
      const outOff = fs.createWriteStream(`${tmpdir}/config/OffScreenBackground.png`, {flags: 'w'});
      outOff.on('close', () => {
        aioLog('Off Screen Background Copy Successful!');
      });
      outOff.on('error', (err: any) => {
  ...
  if (user.mainOps.includes(2)) {
    const inStrbg = fs.createReadStream(`${getBackground}`);
    inStrbg.on('error', (err: any) => {
      aioLog('ERROR: No Background Chosen...', err);
    });
    inStrbg.on('open', () => {
      const out = fs.createWriteStream(`${tmpdir}/config/background.png`, {flags: 'w'});
      out.on('close', () => {
...
function addTweak(twk: any, stat?: boolean) {
  ...
  aioLog(`${twk} added successfully. Status: ${stat}`);
}

function writeTweaksFile() {
  // write stream writes tweaks.txt
  const tweaks = fs.createWriteStream(`${tmpdir}/tweaks.txt`);
...
export const langVar = (id: number) => {
  switch (id) {
    case 0: return 'EN';
    case 1: return 'DE';
    case 2: return 'PL';
...
function writeSpeedoConfigFile(user: any) {
  ...
  let spdStartup = `// speedometer-startup.js\n// Generated By AIO v${app.getVersion()}\n`
  spdStartup += `let enableSmallSbSpeedo = ${user.speedoOps.sml !== 22};\n`
  spdStartup += `let isMPH = ${user.speedoOps.xph == 11};\n`
  spdStartup += `let language = ''${langvar(user.speedoOps.language)}';\n`
  spdStartup += `let fuelEffunit_kml = ${user.speedoOps.fuelEffunit_kml};\n`
  spdStartup += `let black_background_opacity = ${user.speedoOps.black_background_opacity};\n`
  spdStartup += `let original_background_image = ${user.speedoOps.original_background_image};\n`
  spdStartup += `let startAnalog = ${user.speedoOps.startAnalog};\n`
  spdStartup += `let barSpeedometerMod = ${user.speedoOps.barSpeedometerMod};\n`
  spdStartup += `let speedMod = ${user.speedoOps.speedMod};\n`
  spdStartup += `let engineSpeedBar = ${user.speedoOps.engineSpeedBar};\n`
  spdStartup += `let hideSpeedBar = ${user.speedoOps.hideSpeedBar};\n`
  spdStartup += `let tempIsF = ${user.speedoOps.tempIsF};\n`
  spdStartup += `let speedAnimation = ${user.speedoOps.speedAnimation};\n`
  spdStartup += `let analogColor = ${user.speedoOps.analogColor};\n`
  spdStartup += `let speedometerTheme = ${user.speedoOps.speedometerTheme};\n`
  spdStartup += `let sbFuelBar = ${user.speedoOps.sbFuelBar};\n`
  spdStartup += `let sbfbPos = '${user.speedoOps.sbfbPos}';\n`
  spdStartup += `let SbMain = '${user.speedoOps.SbMain}';\n`
  spdStartup += `let SbVal1 = '${user.speedoOps.SbVal1}';\n`
  spdStartup += `let SbVal2 = '${user.speedoOps.SbVal2}';\n`
  fs.writeFileSync(`${varDir}/startupvars.js`, spdStartup)
  let config = fs.createWriteStream(`${tmpdir}/config/speedometer/jci/gui/apps/_speedometer/js/speedometer-startup.js`)
  new appender(speedoConfig).pipe(config)
  config.on('close', () => { console.log("speedometer-startup.js written successfully!") })
*/
  let JSobj = `let spdBottomRows = ${user.spdExtra.barSpeedoRows}; //Number of Bottom Rows\n`;
  JSobj += 'let spdTbl = {\n';
  for (const val in user.spdValues) {
    if (`${user.spdValues[val].pos}` !== null) {
      JSobj += `\t${user.spdValues[val].elmnt}: [${user.spdValues[val].pos}], //${user.spdValues[val].label}\n`;
    }
  }
  JSobj += '}\n';
  JSobj += 'let classicSpeedoTmplt = ';
  ...
  const config = fs.createWriteStream(`${tmpdir}/config/speedometer/speedometer-config.js`);
  new appender(speedoConfig).pipe(config);
  config.on('close', () => {
    // console.log("speedometer-config.js written successfully!")
  });
  fs.writeFileSync(`${tmpdir}/config/speedometer/speedometer-controls.js`, `let spdBtn = ${JSON.stringify(user.multictrl, null, '\t')}`);
  for (const bt in user.barThemeColors) {
    const num = user.barThemeColors[bt].num;
...
function tweakVariables(user: any) {
  mkdirp.sync(`${varDir}`);
  let bak = `KEEPBKUPS=` + (user.backups.org ? `1\n` : `0\n`);
  ...
  if (user.options.includes(13)) {
    let bootAnimations = `BOOTLOGO1=${user.boot.logo1}\n`;
  ...
  if (user.options.includes(1)) {
    let tchscrn = `KEEP_SPEED_RESTRICT=`;
  ...
  if (user.mainOps.includes(9)) {
    let ui = `UI_STYLE_BODY=${user.uistyle.body}\n`;
  ...
  if (user.options.includes(20)) {
    const dateFormat = user.d2sbOps - 1;
    let sc = `DATE_FORMAT=${dateFormat}\n`;
  ...
  if (user.options.includes(17)) {
    let vpops = `VP_SHUFFLE=${user.vpOps.shuffle}\n`;
  ...
  if (user.options.includes(19)) {
    ...
    for (const fb in user.fuelBarColors) {
...
function convert2LF() {
  ...
  crlf.set(`${tmpdir}/tweaks.txt`, 'LF', (err: any, endingType: any) => {
    if (err) {
...
function addTweakDir(twk: any, inst: boolean, cback?: any) {
  const cbackArgs: any[] = [];
...
function addRootFiles(dataDump?: any) {
  try {
...
function aioLog(logMsg: any, err?: any) {
  if (keeplog) {
    AIO_LOG += `- ${logMsg}\n`;
  }
...
function appendAIOlog(logMsg: any) {
  if (keeplog) {
    fs.writeFile(path.resolve(path.join(`${approot}`, `../../${logFileName}.htm`)), logMsg, {flag: 'a'}, (err: any) => {
      if (err) {
        console.warn(err);
      }
  ...
  try {
    ...
    if (usb.length < 1) {
      ...
      for (let j = 0; j < usb.length; j++) {
        const mpLoc = (process.platform === 'win32') ? `${usb[j].mp.replace(/\\/g, '')}` : `${usb[j].mp}`;
...
function copyToUSB(mp: any) {
  const copyingUSB = bootbox.dialog({
...
function unzipSwapfile(dest: any) {
  let nocopy = false;
  if (!dest) {
    nocopy = true;
    dest = `${tmpdir}`;
  ...
  if (copySwapfile) {
    ...
    extract(`${approot}/files/tweaks/config/swapfile/swapfile.zip`, {dir: `${swapdest}`, onEntry: (err) => {
      if (err) {
...
const strtOver = `<button id="startOver" class="w3-round-large w3-btn w3-ripple w3-large w3-hover-white w3-border w3-border-white w3-hover-border-black" onclick="location.reload()" autofocus><span class="icon-space-shuttle"></span>     ${langObj.popupMsgs[17].msg}</button>`;
const viewLog = `<button class="w3-round-large w3-indigo w3-btn w3-ripple w3-hover-cyan w3-large w3-border w3-border-black view-log" title='Compile Log' onclick="$('#opn-mzd-log').click()"><span class='icon-star-full'></span>   ${langObj.popupMsgs[18].msg}</button>`;
const closeApp = `<button class="w3-round-large w3-red w3-btn w3-ripple w3-hover-lime w3-large w3-border w3-border-purple" title="Close The App" onclick="window.close()"><span class="icon-exit"></span>    ${langObj.popupMsgs[19].msg}</button>`;
let cp2usb = `<button class="w3-round-large w3-teal w3-btn w3-ripple w3-hover-pink w3-large w3-border w3-border-indigo" style="letter-spacing:1px" title="Copy These Files To A Blank USB Drive" onclick="openCopyFolder()"><span class="icon-copy2"></span> ${langObj.menu.copytousb.toolTip}</button>`;
const saveBtn = `<button class="w3-round-large w3-purple w3-btn w3-ripple w3-hover-deep-orange w3-large w3-border w3-border-green" style="letter-spacing:.81px" title="Save Options" onclick="$('#save-btn').click()"><span class="icon-floppy-disk"></span> ${langObj.menu.save.toolTip}</button>`;
let openUSB = '';

function finishedMessage(mp?: any) {
  // Finished message
  if (mp) {
...
function casdkAppOptions(apps: any, inst: any) {
  let casdkapps = `APPSIMPLEDASHBOARD=`;
...
function addCASDKapp(add: any, app: any) {
  if (add) {
    copydir(`${builddir}casdk/apps/app.${app}`, `${tmpdir}/casdk/apps/app.${app}`, (err: any) => {
      if (err) {
...
function buildCASDK(user: any, apps: any) {
  addRootFiles();
  ...
  if (user.casdk.inst) {
    ...
    copydir(`${builddir}casdk`, `${tmpdir}/casdk`, (stat: any, filepath: any, filename: any) => {
...
function fullSystemRestore(user: any) {
  addRootFiles();
  ...
  if (fs.existsSync(`${colordir}/Red/jci.zip`)) {
    ...
    extract(`${colordir}/Red/jci.zip`, {dir: `${tmpdir}/config/color-schemes/Red`, onEntry: (err: any) => {
      if (err) {
        aioLog(err, err);
      } else {
...
function buildAutorunInstaller(user: any) {
  ...
  copydir(`${approot}/files/tweaks/cmu-autorun/installer`, `${tmpdir}`, (err: any) => {
    if (err) {
      errFlag = true;
      aioLog('ERROR COPYING AUTORUN FILES', err);
    } else {
      if (user.autorun.id7recovery || user.autorun.serial) {
        copydir(`${approot}/files/tweaks/cmu-autorun/sdcard/recovery`, `${tmpdir}`, (err: any) => {
          if (err) {
            errFlag = true;
...
function addWifiAP(user: any) {
  filesComplete = true;
  opsComplete = true;
  if (user.autorun.autoADB || user.autorun.autoWIFI) {
    copydir(`${approot}/files/tweaks/cmu-autorun/sdcard/recovery-extra`, `${tmpdir}`, (err: any) => {
      if (err) {
        errFlag = true;
        aioLog('ERROR COPYING AUTORUN FILES', err);
      }
...
export const writePresetMessageFile = (result: any) => {
  try {
    ...
    snackbar('Preset Messages Saved!', 0);
  } catch (e) {
    snackbar(`ERROR CREATING PRESET MESSAGES FILE 'blm_msg-system.xml': ${e}`, 0);
  }
};
