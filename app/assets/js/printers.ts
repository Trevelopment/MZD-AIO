...
ipc.on('printer-list', (event, printers) => {
  ...
  if (printers.length > 0) {
    ...
    printers.forEach(function(device, i) {
      ...
      const deviceName = (device.options && device.options['printer-info']) ? device.options['printer-info'] : device.name;
      let buttons = `<button class="printTXTTest" data-printer="${i}">Print TXT Test</button>&nbsp;<button class="printPDFTest" data-printer="${i}">Print PDF Test</button>`;
